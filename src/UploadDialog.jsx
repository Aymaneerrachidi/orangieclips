...
export function UploadDialog({ day, close, onUploaded }) {
  const [file, setFile] = useState(null); const [title, setTitle] = useState(''); const [uploadDay, setUploadDay] = useState(day); const [progress, setProgress] = useState(null); const [error, setError] = useState(''); const [dragging, setDragging] = useState(false); const request = useRef(null);
  ...
  async function submit(e) {
    e.preventDefault(); if (!file) return setError('Choose a video first.'); setError(''); setProgress(0);
    try {
