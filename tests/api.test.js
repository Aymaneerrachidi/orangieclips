...
test('automatic workspace, role isolation, original downloads, daily analytics, reviews and access revocation', async () => {
  ...
  try {
    ...
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{posting_tag:'personal'},owner)).status,200);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{review_note:'Use the shorter intro.'},team)).status,200);
    let tagged = (await (await request('/clips','GET',undefined,sam)).json()).find(c=>c.id===samClip);
    assert.equal(tagged.posting_tag,'personal'); assert.equal(tagged.review_note,'Use the shorter intro.'); assert.equal(tagged.status,'approved');
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{posting_tag:'clip_page'},team)).status,200);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{posting_tag:'personal',review_note:'Clippers cannot edit'},sam)).status,403);
    assert.equal((await request(`/clips/${leeClip}`,'PATCH',{review_note:'Private clip'},sam)).status,404);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{posting_tag:'unknown'},owner)).status,400);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{status:'not_posting',posting_tag:'',review_note:'  '},team)).status,400);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{status:'not_posting',posting_tag:'personal',review_note:'Conflicting plan'},team)).status,400);
    assert.equal((await request(`/clips/${samClip}`,'PATCH',{status:'not_posting',posting_tag:'',review_note:'The audio is too quiet.'},team)).status,200);
    tagged = (await (await request('/clips','GET',undefined,sam)).json()).find(c=>c.id===samClip);
    assert.equal(tagged.status,'not_posting'); assert.equal(tagged.review_note,'The audio is too quiet.'); assert.equal(tagged.posting_tag,'');
    ...
    assert.equal((await (await request('/clips','GET',undefined,sam)).json()).find(c=>c.id===samClip).review_note,'The audio is too quiet.','Notes survive restart');
